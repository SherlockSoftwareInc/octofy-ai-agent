...
from app.utils.sas_normalization import extract_sql_from_sas, looks_like_sas, wrap_sql_as_sas
...
def test_sas_kb_exact_wraps_sql(tmp_path):
    provider = SqliteVecProvider(tmp_path / "sas.sqlite")
    ...
    events = list(generate_sas_builtin(req, stores, llm=FakeLlm()))
    payload = [e for e in events if e.get("type") == "result"][0]["payload"]
    assert payload["discovery_branch"] == "kb_exact"
    assert payload["attempts"] == 0
    assert payload["success"] is True
    assert payload["query_type"] == "sas_code"
    assert "COUNT(*)" in payload["sql"]
    assert "PROC SQL" in payload["sql"]
    assert "CONNECTION TO" in payload["sql"]
    assert any(e.get("type") == "done" for e in events)


def test_sas_force_general_conversational(tmp_path):
    provider = SqliteVecProvider(tmp_path / "sas2.sqlite")
    stores = build_source_stores("s1", provider=provider)
...
def test_sas_priority_validation_failed(tmp_path):
    provider = SqliteVecProvider(tmp_path / "saspins.sqlite")
    stores = build_source_stores("s1", provider=provider)
...
def test_wrap_sql_as_sas_extracts_sql():
    sql = "SELECT COUNT(*) FROM dbo.Customers"
    code = wrap_sql_as_sas(sql)
    assert looks_like_sas(code)
    extracted = extract_sql_from_sas(code)[0].replace("\n", " ").strip()
