...
- **Environment Fallback**: If values are missing in the save request, the backend populates them from active environment variables (`LLM_API_KEY`, `LLM_MODEL`) before saving to `.env`.
...
- **User Guidance**: Added helper text under the API Key input: "Leave empty to use LLM_API_KEY from environment".
